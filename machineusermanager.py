...
try:
    ...
    job_is_running = (
        False  # No pin for machine state defined, always assume no job is active
    )
try:
    machine_is_on = GPIO.input(MACHINE_ON) != INVERT_POWER
except NameError:
    machine_is_on = (
        True  # No pin for machine power defined, always assume machine is on
    )
        ...
        lcd.write_string(
            (
                line.format(
                    ...
                    PRICE_ONCE=(
                        price_once
                        if (int(price_once) != price_once)
                        else int(price_once)
                    ),
                    PRICE_MINUTE=(
                        price_minute
                        if (int(price_minute) != price_minute)
                        else int(price_minute)
                    ),
                )
                + padding
            )[:20]
...
while True:
    ...
    if not machine_is_on:
        ...
        try:
            ...
            logging.exception(
                "Your callback may be malformed or outdated as probably the parameters mismatch"
            )
    if notify is not None:
                    ...
                    if do_cancel:
                        ...
                        if session.create_session(PRICE_ONCE, PRICE_MINUTE):
                            try:
                                ...
                                logging.debug(
                                    "No callback defined for event user_login"
                                )
                            except TypeError:
                                logging.exception(
                                    "Your callback may be malformed or outdated as probably the parameters mismatch"
                            ...
                            if job_is_running:
                                ...
                                if job_active:  # still set from last session
                                    try:
                                        ...
                                        logging.debug(
                                            "No callback defined for event job_resume"
                                        )
                                    except TypeError:
                                        logging.exception(
                                            "Your callback may be malformed or outdated as probably the parameters mismatch"
                            ...
                            try:
                                ...
                                logging.debug(
                                    "No callback defined for event user_login_failed"
                                )
                            except TypeError:
                                logging.exception(
                                    "Your callback may be malformed or outdated as probably the parameters mismatch"
    ...
    while (session_active or OVERRIDE) and machine_is_on:
                ...
                if time_remaining < LOW_CREDIT_MINUTES:
                    if not warning_sent:
                        try:
                            ...
                            logging.debug(
                                "No callback defined for event credit_low_warning"
                            )
                        except TypeError:
                            logging.exception(
                                "Your callback may be malformed or outdated as probably the parameters mismatch"
        ...
        if not OVERRIDE and not session.extend_session():
            ...
            try:
                if job_active:
                    callbacks.credit_runout_interrupt(
                        uid, username, time.time() - job_started
                    )
                else:
                    callbacks.credit_runout(uid, username)
            except NameError:
                logging.debug(
                    "No callback defined for event credit_runout or credit_runout_interrupt"
                )
            except TypeError:
                logging.exception(
                    "Your callback may be malformed or outdated as probably the parameters mismatch"
            ...
            time.sleep(0.3)  # Reduce CPU load by fewer executions
    lock_machine()
